...
def list_processed_agent_data():
    try:
        ...
        return session.execute(select(processed_agent_data)).fetchall()
...
async def create_processed_agent_data(data: List[ProcessedAgentData]):
    try:
        ...
        for i in data:
            post_data = insert(processed_agent_data).values(
            ...
            session.execute(post_data)

            await send_data_to_subscribers(i.agent_data.user_id, i.json())
    ...
    return {"message": "Agent data created"}
...
def delete_processed_agent_data(processed_agent_data_id: int):
    try:
        ...
        session.execute(delete(processed_agent_data).where(processed_agent_data.c.id == processed_agent_data_id))
        session.commit()
